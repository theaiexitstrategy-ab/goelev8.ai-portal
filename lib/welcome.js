...
export async function sendWelcomeForEvent({ client, event }) {
  ...
  const to = toE164(event.contact_phone);
  if (!to) return { sent: false, reason: 'invalid_phone' };

  // Resolve the canonical booking URL for this tenant so templates can use
  // {{booking_url}} instead of hardcoding (and going stale when the page moves).
  let bookingUrl = '';
  try {
  ...
  } catch { /* template just renders empty */ }
