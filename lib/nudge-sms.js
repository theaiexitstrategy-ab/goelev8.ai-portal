...
async function sendOneNudge({ client, contactId, to, text, messageNumber }) {
...
export async function scheduleNudgeSequence({ client, lead }) {
  ...
  const to = lead.phone ? String(lead.phone).trim() : null;
  if (!to || !/^\+?\d[\d\s\-().]{6,}$/.test(to)) return { sent: false, reason: 'no_valid_phone' };
    ...
    if (delayMs === 0) {
      ...
      setTimeout(async () => {
        try {
          ...
          await sendOneNudge({
            client: freshClient, contactId, to, text,
            messageNumber: nudge.message_number
          });
        } catch (e) {
      ...
      // or a Vercel cron) poll for due messages and send them.
