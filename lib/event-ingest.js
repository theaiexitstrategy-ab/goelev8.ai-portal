...
export async function ingestLegacyEventSession({ session }) {
  ...
  const { data: client } = await supabaseAdmin
    ...
    .maybeSingle();
  if (!client) return { ok: false, reason: 'tenant_not_resolved', destination };
  ...
  if (!events?.length) return { ok: false, reason: 'no_events_for_tenant' };
  ...
  if (!matched) {
    ...
    if (published.length !== 1) {
      return { ok: false, reason: 'event_not_matched', charged_cents: charged };
    }
...
export async function backfillLegacyEventSessions({ hoursBack = 720, maxSessions = 100 } = {}) {
  ...
  const out = { scanned: 0, ingested: 0, idempotent: 0, skipped: 0, mismatched: 0, errors: [] };
