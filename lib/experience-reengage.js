...
async function advance({ key, steps, baseTime, lead, state, ctx, stopReason, dry, now, budget }) {
  ...
  try {
    ...
    seq.sending = null; seq.failures = (seq.failures || 0) + 1; seq.last_error = String(e?.message || e).slice(0, 300);
    ...
    for (const row of inquiries || []) {
      try {
        ...
        if (!state.alerted_at) {
          ...
          } catch (e) { out.push(`alert:error:${String(e?.message || e).slice(0, 200)}`); }
        }
