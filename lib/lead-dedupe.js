...
export async function findOrUpsertLead(clientId, payload) {
  ...
  if (existing) {
    ...
    // Structured extras: shallow-merge rather than mergeNonNull's
    // all-or-nothing. A partner who first enquired without a timeline
    // and comes back two weeks later with one should gain the timeline,
    // not be skipped because `payload` was already non-empty. Incoming
    // values win per key; keys only the old submission had survive.
    if (payload.payload && typeof payload.payload === 'object') {
      const prior = (existing.payload && typeof existing.payload === 'object' && !Array.isArray(existing.payload))
        ? existing.payload : {};
      patch.payload = { ...prior, ...payload.payload };
    }
  ...
  if (payload.page_url)     insertRow.page_url     = payload.page_url;
  ...
  if (isMissingColumn) {
    ...
    const { tags, funnel, intent, payload: pl, page_url, ...legacy } = insertRow;
