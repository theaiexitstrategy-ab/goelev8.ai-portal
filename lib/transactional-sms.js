...
import { twilioForClient, estimateSegments, truncateForSms } from './twilio.js';
...
// /api/external/checkout. The body branches by fulfillment method
// (pickup vs shipping) so the buyer gets the right next step. Both
// variants aim for a single GSM-7 segment when the tenant's pickup
// instructions are kept short. Includes a STOP footer for carrier
// + TCPA compliance.
//
// opts.isPickup           — true when the customer chose the $0 pickup
//                            shipping option. Switches the body to the
//                            pickup variant.
// opts.pickupLocation     — clients.pickup_location ("iSlay Studios,
//                            Springfield MO"). Appended to the pickup
//                            body so the buyer knows where to go.
// opts.pickupInstructions — clients.pickup_instructions ("M-F 9-5"
//                            etc). Appended after the location. Tenant
//                            can leave it blank.
// opts.stripePaymentId    — used as credit_ledger.ref_id so the SMS
//                            spend is traceable back to the Stripe
//                            charge that triggered it.
export async function sendOrderReceivedSms({
  client, to, customerName,
  stripePaymentId,
  isPickup,
  pickupLocation,
  pickupInstructions
  ...
  let body;
  if (isPickup) {
    // Pickup body. Location + instructions are both optional from
    // the tenant's perspective; fall back to a generic line when
    // either is missing.
    const loc = pickupLocation
      ? ` at ${pickupLocation}`
      : '';
    const ready = `Your order will be ready for pickup${loc}.`;
    const extras = pickupInstructions
      ? ` ${String(pickupInstructions).trim()}`
      : ` We'll text you when it's ready.`;
    body = `${greet}, ${tenant} here — we got your order! ${ready}${extras} Reply here with any questions. Reply STOP to opt out.`;
  } else {
    body = `${greet}, ${tenant} here — we got your order! We'll ship within 72 hours and USPS usually delivers in 1–2 weeks. Reply here with any questions. Reply STOP to opt out.`;
  }

  return sendTransactionalSms({
    client, to, body,
    ledgerReason: isPickup ? 'order_received_pickup_sms' : 'order_received_sms',
